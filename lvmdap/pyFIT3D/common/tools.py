...
def momana_spec_wave(gas_flux__w, wave, vel, sigma, crval, cdelt,
    ...
    *TODO*: create wavelength intervals withoud crval and cdelt, i.e., set intervals using `wave` and a defined interval in Angstroms.
    ...
    for i_mc in np.arange(0, n_MC):
        ...
        vel_I1 = 0
        f = 1 + vel/__c__
        start_w = wave*f - f_width*__sigma_to_FWHM__*sigma
        end_w = wave*f + f_width*__sigma_to_FWHM__*sigma
...
#            val = val
            w = crval + iz*cdelt
            if (sigma_we > 0 ):
                WE = ((sigma_we*np.sqrt(2*np.pi)))*np.exp(-0.5*((w - wave*f)/sigma_we)**2)
            else:
                WE = cdelt#/np.abs(end_i-start_i)
            #WE = WE**2
            val = val*WE
        ...
        if n_I0 != 0:
            ...
            if sum_val_abs != 0:
                ...
                for iz in np.arange(start_i, end_i):
                    ...
                    if (sigma_we > 0 ):
                        WE1 = (1/(sigma_we*np.sqrt(2*np.pi)))*np.exp(-0.5*((w - wave*f1)/sigma_we)**2)
                    else:
                        WE1 = 1/np.abs(end_i-start_i)
                    if (I0 > 0) & (val > 0):
    ...
    if n_MC > 1:
        ...
        I1 = np.mean(a_I1)
        #
        I2 = __sigma_to_FWHM__*np.sqrt(np.abs(np.median(a_I2_new)))
...
def momana_spec_wave_250427(gas_flux__w, wave, vel, sigma, crval, cdelt,
                     n_MC=10, egas_flux__w=None, flux_ssp__w=None, f_width=1.5):
    """
    It will proceed the moment analysis of emission line centered at `wave`.
    The parameters `crval` and `cdelt` are required in order to generate
    wavelength interval to proceed the analysis.
